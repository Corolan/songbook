...
for i in $(find ./src/songs/ -type f -name "*.tex"); do grep -oP 'rightline{\(\K[^\)}]+' $i | grep -vi capo | grep -vi tonacja; done | sort | uniq -c | sed 's/-"Nieznany"//g' | awk '{first = $1; $1=""; print $0, " \\tab", first " \\newline"}' >> ./tmp/songbook_2.tmp
